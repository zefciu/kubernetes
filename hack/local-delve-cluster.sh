...
WAIT_FOR_URL_API_SERVER=${WAIT_FOR_URL_API_SERVER:-1000}
...
if [ "x$GO_OUT" == "x" ]; then
    "${KUBE_ROOT}/hack/build-go.sh" -gcflags="-N -l" \
        cmd/kubectl \
        cmd/hyperkube
else
...
function start_apiserver {
    ...
    cd cmd/hyperkube
    sudo -E "${GOPATH}/bin/dlv" exec --headless --listen="localhost:8001" --api-version=2 "${GO_OUT}/hyperkube" -- apiserver ${priv_arg} ${runtime_config}\
      --v=${LOG_LEVEL} \
      --cert-dir="${CERT_DIR}" \
